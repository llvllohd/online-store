<template>
  ...
  <nav class="w-full h-16 fixed top-0 bg-gray-900 flex flex-row items-center px-3 z-50" id="header">
    ...
    <!-- Nav Items -->
    <ul class="w-3/5 h-full hidden sm:flex flex-row justify-start items-center space-x-5 md:space-x-10 cursor-pointer">
  ...
  >
    <div v-if="isToggle" class="side-menu sidebar-custom-height bg-gray-900 w-3/6 fixed sm:hidden z-50">
      ...
      <ul class="w-full h-full flex flex-col justify-start items-center pt-11">
        <li v-for="item in navigation" :key="item.name" class="nav text-white font-bold flex w-full items-center justify-center">
          <router-link exact :to="item.path" @click="gotoLinks(item)" class="w-full p-2 flex items-center justify-center">
...
const navigation = [
  ...
  { name: "Logout", icon: "sign-in-alt", path: "login", current: false },
];
